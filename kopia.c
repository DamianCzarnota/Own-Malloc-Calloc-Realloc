...
int main(int argc, char **argv) {
	...
	char *temp = heap_malloc(100);
	char *temp2 = heap_malloc(100);
	char *temp3 = heap_malloc(100);

	display_size_heap();
	
	heap_free(temp2);
	
	temp2 = heap_malloc(90);
...
void* heap_malloc(size_t count){
  ...
  while( zm->flaga==1||zm->size<count){
    zm=zm->next;
    if(!zm){
      if(heap_get_more_memory()) {pthread_mutex_unlock(&mutex);return NULL;}
...
void* heap_realloc(void *memblock,size_t size){
  pthread_mutex_lock(&mutex);
  if(heap_validate()) {pthread_mutex_unlock(&mutex);return NULL;}
  if(!memblock) {pthread_mutex_unlock(&mutex);return heap_malloc(size);}
  if(!size) {pthread_mutex_unlock(&mutex);heap_free(memblock); return NULL;}
...
void* heap_realloc_aligned(void *memblock,size_t size){
  pthread_mutex_lock(&mutex);
  if(heap_validate()) {pthread_mutex_unlock(&mutex); return NULL;}
  if(!memblock) {pthread_mutex_unlock(&mutex); return heap_malloc_aligned(size);}
  if(!size) {pthread_mutex_unlock(&mutex); heap_free(memblock); return NULL;}
  struct node_t *zm=(struct node_t*)((char*)memblock-sizeof(struct node_t)-FENCE_SIZE*FENCE_AMMOUNT);
  uint64_t liczba=PAGE_SIZE;
